...
public sealed class DefaultMeshBackend : IGeometryBackend
{
    public MeshModel WeldVertices(MeshModel input, float epsilonMm, out int mergedVertices)
    {
        var vertices = ToVectorList(input.Vertices);
        var welded = WeldVertices(vertices, input.Indices, epsilonMm);
        mergedVertices = vertices.Count - welded.Vertices.Count;
        return BuildMesh(welded.Vertices, ToTriangles(welded.Indices), input.Units);
    }

    public MeshModel RemoveDegenerateFaces(MeshModel input, float epsilonMm, out int removedFaces)
    {
        var vertices = ToVectorList(input.Vertices);
        var (triangles, removed) = RemoveDegenerateTriangles(vertices, input.Indices, epsilonMm);
        removedFaces = removed;
        return BuildMesh(vertices, triangles, input.Units);
    }

    public MeshModel RemoveDuplicateFaces(MeshModel input, out int removedFaces)
    {
        var vertices = ToVectorList(input.Vertices);
        var triangles = ToTriangles(input.Indices);
        var (deduped, removed) = RemoveDuplicateTriangles(triangles);
        removedFaces = removed;
        return BuildMesh(vertices, deduped, input.Units);
    }

    public MeshModel FixNormalsAndOrientation(MeshModel input)
    {
        var vertices = ToVectorList(input.Vertices);
        var triangles = ToTriangles(input.Indices);
        var oriented = FixWindingConsistency(triangles, vertices);
        return BuildMesh(vertices, oriented, input.Units);
    }

    public MeshModel FillSmallHoles(MeshModel input, float closeRadiusMm, out int addedFaces)
    {
        var vertices = ToVectorList(input.Vertices);
        var triangles = ToTriangles(input.Indices);
        var (closed, added) = CloseSmallHoles(triangles, vertices, closeRadiusMm);
        addedFaces = added;
        return BuildMesh(vertices, closed, input.Units);
    }

    public MeshModel RemoveTinyShells(MeshModel input, float tinyShellThresholdMm, out int removedFaces)
    {
        var vertices = ToVectorList(input.Vertices);
        var triangles = ToTriangles(input.Indices);
        var (withoutTinyShells, removed) = RemoveTinyShells(triangles, vertices, tinyShellThresholdMm);
        removedFaces = removed;
        return BuildMesh(vertices, withoutTinyShells, input.Units);
    }
    ...
    private static List<(int A, int B, int C)> ToTriangles(int[] indices)
    {
        var triangles = new List<(int A, int B, int C)>(indices.Length / 3);
        for (var i = 0; i + 2 < indices.Length; i += 3)
        {
            triangles.Add((indices[i], indices[i + 1], indices[i + 2]));
        }
